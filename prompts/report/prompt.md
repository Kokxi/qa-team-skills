...
- **写入**：输出中的报告类型、时间段、关键指标等，自动按 `memory/schema/report.json` 结构化存入报告库
- **读取**：自动检索报告库（`memory/data/report/`）中同类型的历史报告，辅助同比/环比趋势分析；自动检索用例库和缺陷库，汇总数据生成报告

## 输出前自检（必须逐条核对，不通过不输出）

- [ ] 报告类型是否与用户指定的匹配（日报/周报/阶段/季度/专项）？
- [ ] 所有数据是否有来源标注？（文本输入 / 文件名 / 系统名称）
- [ ] 缺失数据是否用 `-` 标注？（不得编造数字）
