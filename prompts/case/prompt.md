...
- **通过 `/qa` 入口调用时**：自动从评审库（`memory/data/review/`）检索同一模块的最近评审记录，将问题清单注入"评审问题清单"输入，无需手动粘贴
- **直接调用时**：手动粘贴 `/qa-prd` 输出的问题表格，AI 自动将问题转化为对应类型的测试用例

评审中发现的安全/边界/性能遗漏将自动转化为对应类型的用例。
...
- [ ] 涉及输入格式的场景（登录/注册等），是否覆盖了字符集、长度边界、空输入？
- [ ] 安全测试中是否包含信息泄露防护检查（错误提示是否通用化）？
