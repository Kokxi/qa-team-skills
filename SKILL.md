...
description: >
  ...
  覆盖需求评审、测试用例设计、AI Agent专项测试、缺陷根因分析、自动生成测试报告、版本准入与团队管理六大场景，适配测试工程师、测试经理日常工作。
metadata:
  author: Kokxi
...
| 指令 | 能力 | 版本 |
|------|------|------|
| `/qa-prd` | 11维度评审 + 业务分层建议 + 澄清问题 | V1.3.1 |
| `/qa-case` | 6类型 × 9黑盒方法 + 业务分层（核心/体验/增值） | V1.3.1 |
| `/qa-agent` | 16维度 Agent 专项（含 RAG 3维度） | V1.3.1 |
| `/qa-bug` | 质量评估 + 根因分析 + 批量模式 + 缺陷关联 | V1.3.1 |
| `/qa-report` | 5种报告 + API拉取 + 安全/兼容性专项 | V1.3.1 |
| `/qa-team` | 11项管理能力 + 子能力路由 | V1.3.1 |
