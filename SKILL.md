...
version: v1.6.2
license: MIT
description: 当用户需要进行需求评审、测试用例设计、缺陷分析、Bug分析、根因分析、测试报告、周报、日报、团队管理、准入准出、质量评估、Agent测试、AI测试、RAG测试、回归测试、探索性测试时，使用此技能——支持自然语言下达测试任务（如"对支付接口做全量回归并输出缺陷报告"），即使没有明确说指令名。包含统一入口 /qa + 8 个标准化指令 + 记忆模块，覆盖需求评审到探索性测试全流程，并支持自动规划：用户只说"测一下支付接口"，AI 自动判断最佳步骤组合。
author: Kokxi
type: user
...
| 意图路由不确定 / 需要子能力匹配 | `prompts/qa/intent-rules.md` | 单步/多步/自动规划匹配规则（仅在 `/qa` 路由犹豫时读取） |
...
| 指令 | 能力 | 版本 |
|------|------|------|
| `/qa` | 统一入口：意图解析 + 任务编排 + 记忆管理 + **自动规划（v1.5 新增）** | V1.6.2 |
...
| `/qa-explore` | 探索性测试：定义探索任务 → 引导执行 → 记录发现 → 沉淀经验 | V1.6.2 |
