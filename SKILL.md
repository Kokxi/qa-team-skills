---
name: qa-team-skills
description: 团队统一技能——将 AI Agent 嵌入软件测试全流程。6 大指令覆盖：需求评审、用例设计、AI Agent 专项测试、缺陷分析、报告生成、团队管理。适用于任意行业和产品类型。
metadata:
  type: user
  version: v1.2.0
  author: QA Team
  security: "无外部依赖，纯 Prompt 工程，不涉及网络请求或文件写入"
---
...
| 指令 | 能力 | 版本 |
|------|------|------|
| `/qa-prd` | 11维度评审 + 业务分层建议 + 澄清问题 | V1.0.1 |
| `/qa-case` | 6类型 × 9黑盒方法 + 业务分层（核心/体验/增值） | V1.0.1 |
| `/qa-agent` | 16维度 Agent 专项（含 RAG 3维度） | V1.0.1 |
| `/qa-bug` | 质量评估 + 根因分析 + 批量模式 + 缺陷关联 | V1.0.1 |
| `/qa-report` | 5种报告 + API拉取 + 安全/兼容性专项 | V1.0.1 |
| `/qa-team` | 11项管理能力 + 子能力路由 | V1.0.1 |
