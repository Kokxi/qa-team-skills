...
version: v1.5.0
license: MIT
description: 为测试团队设计的统一 AI 辅助能力，支持自然语言下达测试任务（如"对支付接口做全量回归并输出缺陷报告"）。当用户提到需求评审、测试用例、缺陷分析、Bug分析、根因分析、测试报告、周报、日报、团队管理、准入准出、质量评估、Agent测试、AI测试、回归测试时，优先使用此技能——即使他们没有明确说指令名。包含统一入口 /qa + 6 个标准化指令 + 记忆模块，覆盖需求评审到团队管理全流程。v1.5 新增自动规划能力——用户只说"测一下支付接口"，AI 自动判断最佳步骤组合。
author: Kokxi
type: user
trigger: ["需求评审", "测试用例", "用例设计", "缺陷分析", "Bug分析", "根因分析", "测试报告", "日报", "周报", "测试管理", "准入准出", "质量评估", "Agent测试", "AI测试", "RAG测试", "全量回归", "测试任务", "探索性测试", "探索测试", "自由探索"]
security: 纯 Prompt 技能，自身不发起网络请求。记忆模块仅在本地文件系统读写，不涉及网络传输。/qa-report 可通过 Jira/禅道等系统数据生成报告。
...
| 指令 | 能力 | 版本 |
|------|------|------|
| `/qa` | 统一入口：意图解析 + 任务编排 + 记忆管理 + **自动规划（v1.5 新增）** | V1.5.0 |
...
| `/qa-explore` | 探索性测试：定义探索任务 → 引导执行 → 记录发现 → 沉淀经验 | V1.5.0 |
