...
author: Kokxi
type: user
trigger: ["需求评审", "测试用例", "用例设计", "缺陷分析", "Bug分析", "根因分析", "测试报告", "周报", "准入准出", "质量评估", "Agent测试", "AI测试", "RAG测试", "全量回归", "测试任务", "探索性测试", "探索测试"]
# 注意：以上 trigger 仅供意图匹配。模糊请求（如"看趋势""写报告""进度"）会先向用户确认意图再执行，避免误触发
